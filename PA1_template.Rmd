...
The median number of steps is $`r raw_med`$ and the mean is $`r raw_mean`$
...
------------- | -------------|------------|
Raw           | $`r raw_mean`$ |$`r raw_med`$ |   
Imputed       |$`r imp_mean`$  |$`r imp_med`$ |
Difference    |$`r raw_mean-imp_mean`$|$`r raw_med-imp_med`$

Including imputed values has no real inpact on the mean or median

The total number of steps increased $`r round(sum(daysum1$totstep)/sum(daysum$totstep,na.rm=T),2)`$ times
